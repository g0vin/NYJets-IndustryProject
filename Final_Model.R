...
tickets_all$season_year

tickets_all$log_Gavin <- log(tickets_all$season_year)
...
model3 <- lm(te_purchase_price~I(log(season_year))+PC1+Time_Before+Opponent+
...
new_data <- test_data %>% 
  dplyr::select(season_year,Time_Before,Opponent,PC1,
  ...
  filter(Opponent == "New England Patriots")
